...
data class LogInToDesktop(
    ...
    suspend operator fun invoke(
        ...
        try {
            db.collection(
                FirebaseCollectionPath.USER.path
            ).document(uid).update("systemUid", model.systemUid).get()
            db.collection(
                FirebaseCollectionPath.USER.path
            ).document(uid).update("desktopLogInDetails", model).get()
            uid
        } catch (e: Exception) {
            throw Exception("Failed to login to desktop: ${e.message}")
        }
    }
}